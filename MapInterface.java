...
public class MapInterface extends JFrame implements Observer {
    ...
    /**
     * A method to clear the map. We admit that we can clear tha map only if there is no algo occuring
     * @return the button to clear the panel
     */
    ...
    /**
     * 
     * @return A panel containing all options
     */
    ...
    private JSlider getGridWidth(){
    	if(grid_width == null){
    		grid_width = getSlider(1, 5, 1);
    		
    		grid_width.addChangeListener(new ChangeListener() {
				
				@Override
				public void stateChanged(ChangeEvent e) {
					nw.setNbRows(grid_width.getValue());
				}
    ...
    private JSlider getGridHeight(){
    	if(grid_height == null){
    		grid_height = getSlider(1, 5, 1);
    		
    		grid_height.addChangeListener(new ChangeListener() {
				
				@Override
				public void stateChanged(ChangeEvent e) {
					nw.setNbColumns(grid_height.getValue());
				}
   ...
   private JMenuItem getJMenuItem36() {
    ...
    private JMenuItem getNInit() {
	if (nInit == null) {
	    nInit = new JMenuItem();
	    nInit.setText("Random weigths");
	    nInit.addActionListener(new java.awt.event.ActionListener() {
		    public void actionPerformed(java.awt.event.ActionEvent e) {
			//teste l'existence de donnees
			dSomInitialize = false;
			somInitialize  = false;
